...
router.post("/", auth, upload.array("images", 5), async (req, res) => {
  try {
    ...
    const { name, description, capacity, address, price, cabinType, isCafe, isChamber, cabin, tableNumber, openTime, closeTime, is24x7 } = req.body;
    ...
    let pricingPlans = [];
    try {
      pricingPlans = req.body.pricingPlans ? (typeof req.body.pricingPlans === "string" ? JSON.parse(req.body.pricingPlans) : req.body.pricingPlans) : [];
    } catch (parseError) {
    ...
    let seats = [];
    try {
      seats = req.body.seats ? (typeof req.body.seats === "string" ? JSON.parse(req.body.seats) : req.body.seats) : [];
    } catch (parseError) {
      console.error("❌ Error parsing seats:", parseError);
      return res.status(400).json({ message: "Invalid seats format" });
    }

    // ✅ If no seats provided, auto-generate from capacity
    const numCapacity = Number(capacity) || 1;
    if ((!seats || seats.length === 0) && numCapacity > 0) {
      seats = [];
      for (let i = 1; i <= numCapacity; i++) {
        seats.push({ name: `Seat ${i}`, number: i });
      }
    }
    ...
    const newCabin = new Cabin({
      ...
      price: Number(price),
      cabinType: cabinType || 'normal',
      cabin: cabin || tableNumber || "",
      tableNumber: tableNumber || cabin || "",
      isCafe: isCafe === true || isCafe === "true",
      isChamber: isChamber === true || isChamber === "true",
    ...
    console.log("Saving new cabin/table to database...");
...
router.post('/createcabinorder', auth, async (req, res) => {
  try {
    ...
    console.log("📝 Creating order for cabin:", cabinId);
    ...
    const order = new CabinOrder({
      cabin: cabinId,
      owner: userId,  // ✅ FIXED: Use userId instead of ownerId
      baseAmount: baseAmount,
      gstAmount: gstAmount,
      amount: totalAmount,
      gstRate: GST_RATE,
    ...
    console.log("─────────────────────────────────────────────");
    console.log("   🆔 Order ID:", order._id);
    console.log("   👤 Owner ID:", userId);  // ✅ FIXED: Use userId
    console.log("   💰 Amount: ₹", totalAmount);
    console.log("   📅 Expiry (1 month):", expiryDate);
    console.log("─────────────────────────────────────────────");
    console.log("═════════════════════════════════════════════");
...
router.put("/:id", auth, upload.array("images", 5), async (req, res) => {
  try {
    ...
    const { name, description, capacity, address, price, cabinType, isCafe, isChamber, cabin: cabinSpec, tableNumber, openTime, closeTime, is24x7, isActive } = req.body;

    if (name) cabin.name = name;
    if (description !== undefined) cabin.description = description;
    if (capacity) cabin.capacity = capacity;
    if (address) cabin.address = address;
    if (cabinType) cabin.cabinType = cabinType;
    if (cabinSpec !== undefined) cabin.cabin = cabinSpec;
    if (tableNumber !== undefined) cabin.tableNumber = tableNumber;
    if (isCafe !== undefined) cabin.isCafe = isCafe === true || isCafe === "true";
    if (isChamber !== undefined) cabin.isChamber = isChamber === true || isChamber === "true";
    if (openTime) cabin.openTime = openTime;
    if (closeTime) cabin.closeTime = closeTime;
    if (is24x7 !== undefined) cabin.is24x7 = is24x7 === true || is24x7 === "true";
    if (isActive !== undefined) cabin.isActive = isActive === true || isActive === "true";

    if (req.body.amenities) {
      try {
        cabin.amenities = typeof req.body.amenities === "string" ? JSON.parse(req.body.amenities) : req.body.amenities;
      } catch (e) {
    ...
    if (req.body.pricingPlans) {
      try {
        const plans = typeof req.body.pricingPlans === "string" ? JSON.parse(req.body.pricingPlans) : req.body.pricingPlans;
        cabin.pricingPlans = plans;
      } catch (e) {
        return res.status(400).json({ message: "Invalid pricingPlans format" });
      }
    ...
    res.json({ success: true, message: "Updated successfully", cabin });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
// ======================
// GET CAFE TABLES BY OWNER ID (Public - No Auth Required)
// ======================
router.get('/my-cafes/:ownerId', async (req, res) => {
  try {
    const { ownerId } = req.params;

    console.log("📋 Fetching cafe tables for owner:", ownerId);
    ...
    // ✅ Find all cabins where:
    // 1. Owner matches the ID
    // 2. Either isCafe is true OR it's a table/cafe space
    const cabins = await Cabin.find({
      owner: ownerId,
      $or: [
        { isCafe: true },
        { isChamber: { $ne: true } },
        { cabin: { $exists: true, $ne: "" } },
        { tableNumber: { $exists: true, $ne: "" } }
      ]
    }).sort({ createdAt: -1 });

    console.log(`✅ Found ${cabins.length} cafe tables for owner`);

    // ✅ Get active order status for each cabin
    const cabinsWithStatus = await Promise.all(
      cabins.map(async (cabin) => {
        const activeOrder = await CabinOrder.findOne({
          cabin: cabin._id,
          status: 'active',
          paymentStatus: 'completed'
        });
        ...
        return {
          ...
          orderStatus: activeOrder ? 'active' : (pendingOrder ? 'pending' : 'none')
        };
      })
    );
    ...
    res.status(200).json({
      success: true,
      count: cabinsWithStatus.length,
      cabins: cabinsWithStatus,
      stats: {
        total: cabinsWithStatus.length,
        active: cabinsWithStatus.filter(c => c.isActive === true).length,
        inactive: cabinsWithStatus.filter(c => c.isActive === false).length,
        exclusive: cabinsWithStatus.filter(c => c.cabinType === 'exclusive').length,
        normal: cabinsWithStatus.filter(c => c.cabinType === 'normal' || !c.cabinType).length,
        withExpiry: cabinsWithStatus.filter(c => c.expiryDate).length
      }
    });
    ...
    res.status(500).json({
      success: false,
      error: "Failed to fetch cafe tables",
      details: error.message
    });
  }
});
